...
// Index returns the list of all Attributes in the Campaign associated with campID.
...
// Get returns the Attribute associated with atrID from the Campaign
...
// Create creates a new Attribute in the Campaign associated with campID using
// the provided SimpleAttribute data.
...
func (as *AttributeService) Create(campID int, entID int, atr SimpleAttribute) (*Attribute, error) {
	...
	err = as.client.post(end, bytes.NewReader(b), &wrap)
	if err != nil {
...
// Update updates an existing Attribute associated with atrID from the
...
func (as *AttributeService) Update(campID int, entID int, atrID int, atr SimpleAttribute) (*Attribute, error) {
	...
	err = as.client.put(end, bytes.NewReader(b), &wrap)
	if err != nil {
