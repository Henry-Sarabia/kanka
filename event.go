...
func (sevent SimpleEvent) MarshalJSON() ([]byte, error) {
	if blank.Is(sevent.Name) {
		return nil, fmt.Errorf("cannot marshal SimpleEvent into JSON with a missing Name")
	}

	type alias SimpleEvent
	return json.Marshal(alias(sevent))
}
...
func (events *EventService) Index(campID int, sync *time.Time) ([]*Event, error) {
	end, err := EndpointCampaign.ID(campID)
	if err != nil {
		return nil, fmt.Errorf("invalid Campaign ID: %w", err)
	}
	end = end.Concat(events.end)
	...
	err = events.client.get(end, &wrap)
	if err != nil {
...
func (events *EventService) Get(campID int, evtID int) (*Event, error) {
	end, err := EndpointCampaign.ID(campID)
	if err != nil {
		return nil, fmt.Errorf("invalid Campaign ID: %w", err)
	}
...
func (events *EventService) Create(campID int, evt SimpleEvent) (*Event, error) {
	end, err := EndpointCampaign.ID(campID)
	if err != nil {
		return nil, fmt.Errorf("invalid Campaign ID: %w", err)
	}
	...
	err = events.client.post(end, bytes.NewReader(b), &wrap)
	if err != nil {
...
func (events *EventService) Update(campID int, evtID int, evt SimpleEvent) (*Event, error) {
	end, err := EndpointCampaign.ID(campID)
	if err != nil {
		return nil, fmt.Errorf("invalid Campaign ID: %w", err)
	}
	...
	err = events.client.put(end, bytes.NewReader(b), &wrap)
	if err != nil {
...
func (events *EventService) Delete(campID int, evtID int) error {
	end, err := EndpointCampaign.ID(campID)
	if err != nil {
